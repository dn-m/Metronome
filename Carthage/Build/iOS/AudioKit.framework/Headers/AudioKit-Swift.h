...
+ (AVAudioTime * _Nonnull)secondsToAVAudioTimeWithHostTime:(uint64_t)hostTime time:(double)time;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
- (nonnull instancetype)initWithAvAudioNode:(AVAudioNode * _Nonnull)avAudioNode attach:(BOOL)attach SWIFT_UNAVAILABLE;
@end
...
- (AKCustomUgen * _Nonnull)duplicate;
...
+ (AKDevice * _Nullable)outputDevice;
/**
...
/**
  ...
  touches
  \endcode until such time as all
...
/**
  ...
  \endcode which allows the delegate to receive messages relating to
  individual touches. The \code
  delegate
  \endcode property can be set to a class
  implementing \code
